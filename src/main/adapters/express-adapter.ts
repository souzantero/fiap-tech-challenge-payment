...
export type AdaptMiddlewareOptions = {
  accessTokenQuery?: boolean;
};

export const adaptMiddleware = <T>(
  middleware: HttpController<T>,
  options?: AdaptMiddlewareOptions,
) => {
  const accessTokenQuery = options?.accessTokenQuery ?? false;
